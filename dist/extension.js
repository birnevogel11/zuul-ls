...
async function activate(context) {
    let disposable = vscode_1.commands.registerCommand("helloworld.helloWorld", async (uri) => {
        ...
        const url = vscode_1.Uri.parse('/home/victor/Documents/test-dir/nrs/another.nrs');
    ...
    const traceOutputChannel = vscode_1.window.createOutputChannel("Zuul Language Server trace");
    const command = process.env.SERVER_PATH || "zuul-ls";
    const run = {
        command,
        options: {
            env: {
    ...
    client = new node_1.LanguageClient("zuul-language-server", "zuul language server", serverOptions, clientOptions);
